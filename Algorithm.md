...
1.小车走直线，定速度，定位置(不用双环PID，定位置就是差速+位置环，定速度就是差速+速度环)
    1.1 双轮差速环PID
...
2.灰度循迹算法
    赋予八路灰度传感器的八个通道相应的权重，根据权重计算出当前的循迹方向
        例如：
            通道1：-1.0f
            通道2：-0.7f
            通道3：-0.5f
            ...
            通道8：1.0f
        计算出当前的偏转方向为：
    LineTurn = 通道1*（-1.0f） + 通道2*（-0.7f） + 通道3*（-0.5f） + 通道4*（-0.3f） + 通道5*（0.3f） + 通道6*（0.5f） + 通道7*（0.7f） + 通道8*（1.0f）
    通过调整小车左右轮的转速来修正循迹方向，保持小车在轨道上行驶。
    当LineTurn为0时，小车保持直线行驶，否则根据LineTurn的正负，调整左右轮的转速，使小车向左或向右转。