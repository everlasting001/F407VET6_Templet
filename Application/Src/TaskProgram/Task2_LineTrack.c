...
#define TASK2_ADJUST_DISTANCE_MM      90.0f
...
void Task2_LineTrack_Init(void)
{
    ...
    move_ctrl.slow_pwm         = TASK2_SLOW_PWM;

    /* 4. 设置顺时针转弯方向 (右转, Yaw: 0 → -90°) */
    move_ctrl.turn_direction = -1;
    ...
    DebugPrintf_Print(&dbg_printf,
        "=== Task2: Square Border Clockwise Track Start ===\r\n");
    DebugPrintf_Print(&dbg_printf,
        "  Edges: %d, BasePWM: %.0f, KLine: %.0f, TurnPWM: %.0f, Dir:CW\r\n",
        SQUARE_TARGET_EDGES,
...
void Task2_LineTrack_Loop(void)
{
    ...
    DebugPrintf_Print(&dbg_printf,
        "[Task2] Edge:%d/4 State:%s LT:%.1f CH:0x%02X "
        "LPWM:%.0f RPWM:%.0f\r\n",
        edge + 1, sname,
