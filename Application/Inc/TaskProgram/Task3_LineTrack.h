/**
  ******************************************************************************
  * @file    Task3_LineTrack.h
  * @brief   Task3 — 正方形边框循迹一圈 (100cm×100cm)
  *
  * @details
  * 基于 Framework 层 MoveControl 巡线状态机，实现对正方形边框的完整循迹。
  * 小车在每条 100cm 直线上循线，到达直角拐角时通过灰度传感器检测路口，
  * 自动执行 90° 直角转弯（陀螺仪 Yaw 闭环），完成 4 条边后自动停车。
  *
  * === 使用模块 ===
  ...
  *
  * === 三分法结构 ===
  * - Task3_LineTrack_Init()       — main() USER CODE BEGIN 2, 初始化循迹参数
  * - Task3_LineTrack_Loop()       — main() USER CODE BEGIN 3, 状态监控与调试输出
  * - Task3_LineTrack_IRQHandler() — 中断回调 (预留，当前循迹在 MoveControl ISR 中执行)
  *
  ******************************************************************************
  */

#ifndef __TASK3_LINE_TRACK_H__
#define __TASK3_LINE_TRACK_H__
