...
#define DEFAULT_FAKE_TURN_THRESHOLD_MM  750.0f  /**< 假路口过滤距离阈值 */
                      ...
                      Encoder_t *encoder_left, Encoder_t *encoder_right)
{
    ...
    PID_Init(&ctrl->turn_pid, DEFAULT_TURN_KP, 0.05f, 0.08f,
             200.0f, DEFAULT_TURN_PWM);
    ctrl->turn_direction           = 1;  /* 默认 CCW (逆时针/左转) */
    ctrl->turn_angle               = 90.0f;  /* 默认 90° 转弯, Task2 可在第4路口改为 180° */
    ctrl->turn_reverse_boost       = 1.0f;   /* 反转轮补偿系数, 180°时可加大 */
    ctrl->turn_other_boost         = 1.0f;   /* 另一轮补偿系数 */
    ...
    ctrl->initial_turn_setup_done  = 0;
...
void MoveControl_LineTrackUpdate(MoveControl_t *ctrl)
{
        ...
        if (ctrl->edge_count == 0
            ...
            if (ctrl->intersection_cnt >= ctrl->intersection_threshold) {
                ...
                if (ctrl->encoder_left && ctrl->encoder_right) {
                    float dl = Encoder_GetDistance(ctrl->encoder_left);
                    float dr = Encoder_GetDistance(ctrl->encoder_right);
                    traveled_mm = (dl + dr) * 0.5f;
                }

                if (!ctrl->first_intersection
                    ...
                    /* 根据检测侧自动决定转弯方向: 右边多→左转(CCW), 左边多→右转(CW) */
                    if (right_cnt > left_cnt) {
                        ctrl->turn_direction =  1;  /* CCW */
                    } else if (left_cnt > right_cnt) {
                        ctrl->turn_direction = -1;  /* CW */
    ...
    case LINE_STATE_TURNING: {
        ...
        if (fabsf(yaw_error) <= ctrl->turn_tolerance) {
            ...
            } else if (ctrl->task_id == TASK_ID_4) {
                /* Task4: 第1边沿90°转弯完成 → 400pwm循迹500ms */
                ctrl->edge_count++;
                ctrl->line_state = LINE_STATE_TASK4_POST_FOLLOW;
                ctrl->start_tick = HAL_GetTick();
            } else {
                ctrl->edge_count++;
            ...
            /* 双轮补偿: 180°掉头时反转轮加力+另一轮减速, 防绕单侧转 */
            if (ctrl->turn_reverse_boost != 1.0f) {
                if (ctrl->turn_direction == 1) {
                    pwm_l *= ctrl->turn_reverse_boost;  /* CCW: 左轮反转 */
                    pwm_r *= ctrl->turn_other_boost;    /* CCW: 右轮补偿 */
                } else {
                    pwm_r *= ctrl->turn_reverse_boost;  /* CW: 右轮反转 */
                    pwm_l *= ctrl->turn_other_boost;    /* CW: 左轮补偿 */
                }
            }
    ...
    /* ---- 状态 7: 初始180°顺时针掉头 (Task4) ---- */
    case LINE_STATE_INITIAL_TURN: {
        if (!ctrl->initial_turn_setup_done) {
            ctrl->initial_turn_setup_done = 1;
            if (ctrl->gyro != NULL) {
                ctrl->turn_start_yaw = Gyro_GetYaw(ctrl->gyro);
            } else {
                ctrl->turn_start_yaw = 0.0f;
        ...
        if (fabsf(yaw_error) <= ctrl->turn_tolerance) {
            ...
            if (ctrl->turn_reverse_boost != 1.0f) {
                if (ctrl->turn_direction == 1) {
                    pwm_l *= ctrl->turn_reverse_boost;
                    pwm_r *= ctrl->turn_other_boost;
                } else {
                    pwm_r *= ctrl->turn_reverse_boost;
                    pwm_l *= ctrl->turn_other_boost;
                }
            }
    ...
    /* ---- 状态 8: Task4 第1边沿90°转弯后 400pwm循迹 500ms ---- */
    case LINE_STATE_TASK4_POST_FOLLOW: {
        float base = 400.0f;
        float pwm_l = base + line_turn * ctrl->k_line;
        float pwm_r = base - line_turn * ctrl->k_line;
        ...
        if (HAL_GetTick() - ctrl->start_tick >= 500) {
            DCMotor_Stop(ctrl->motor_left);
            DCMotor_Stop(ctrl->motor_right);
            ctrl->state = MOVE_STATE_COMPLETE;
            ctrl->buzzer_beep_flag = 2;
...
void MoveControl_SetLineTrack(MoveControl_t *ctrl, LineSensor_t *sensor)
{
    ...
    if (ctrl->task_id == TASK_ID_4) {
        /* Task4: 先做初始180°顺时针掉头, 再做巡线 */
        ctrl->line_state            = LINE_STATE_INITIAL_TURN;
        ctrl->turn_direction        = -1;      /* CW 顺时针 */
        ctrl->turn_angle            = 180.0f;
        ctrl->turn_reverse_boost    = 1.3f;    /* CW: 反转轮补偿 */
        ctrl->turn_other_boost      = 1.0f;    /* CW: 另一轮 */
        ctrl->initial_turn_setup_done = 0;
    } else {
        ctrl->line_state         = LINE_STATE_FOLLOWING;
...
/**
  * @brief  设置假路口过滤距离阈值
  * @param  ctrl  运动控制器
  * @param  mm    距离阈值 (mm), 转弯后行驶不足此距离的路口视为假路口
  */
void MoveControl_SetFakeTurnThreshold(MoveControl_t *ctrl, float mm)
{
    if (ctrl == NULL) return;
    ctrl->fake_turn_threshold_mm = mm;
}
