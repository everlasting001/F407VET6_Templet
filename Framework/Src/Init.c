...
/**
  * @brief  Framework 层运行循环
  *
  * 调度所有模块的周期性运行, 包括运动控制更新。
  * 在 main.c 的 while(1) 循环中调用, 保持快速返回。
  */

void Framework_Run(void){
    Motor_Run(&left_motor.base);
    Motor_Run(&right_motor.base);
}
...
/**
  ...
  *   2. MoveControl_Update: 级联 PID 控制 (位置环 + 差速修正 + 速度环 → PWM)
  *
